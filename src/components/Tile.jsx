...
export default function Tile({logo, company, newPost, featured, position, postedAt, contract, location, role, level, tools, languages, key}) {
    ...
    return (
        <div className={ featured ? 'bg-white w-4/5 pb-2 mx-auto shadow-lg border-l-8 border-l-aqua': 'bg-white w-4/5 pb-2 mx-auto shadow-lg' }>
          ...
          <section className='flex flex-wrap gap-3 ml-6 my-3 mx-2'>
            <div className='bg-zinc-200 p-1.5 text-aqua'>{role}</div>
            <div className='bg-zinc-200 p-1.5 text-aqua'>{level}</div>
            { languages.map((language) => <div key={languages.id} className='bg-zinc-200 p-1.5 text-aqua'>{language}</div>) }
            { tools.map((tool) => <div key={tools.id} className='bg-zinc-200 p-1.5 text-aqua'>{tool}</div>) }
          </section>
        </div>
    )
